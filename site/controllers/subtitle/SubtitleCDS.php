<?php
	...
	class CSubtitleCDS{
		...
		function SubtitleCDS_content(){
				$content = array
				  (
				  array("{title}", $GLOBALS['system']->l("subtitle_SubtitleCDS_title")),
		...
		function GetCDS(){
			$cds = $GLOBALS['system']->GetSubtitleCDS($GLOBALS['vars']['sid']);
			$res = "";
			for ($i=0; $i < count($cds); $i++) {
				...
				$res .= "<td><a href='index.php?page=".$GLOBALS['vars']['page']."&sec=".$GLOBALS['vars']['sec']."&sid=".$GLOBALS['vars']['sid']."&cid=".$cds[$i]['id']."'>".$GLOBALS['system']->l("subtitle_SubtitleCDS_edit")."</a></td>";
